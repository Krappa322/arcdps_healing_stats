...
TEST_P(XevtcLogTestFixture, druid_solo)
{
	...
	expectedSkills.Add(31894, "Rejuvenating Tides", combatTime, 15362, 20, std::nullopt, 0);
	expectedSkills.Add(718, "Regeneration", combatTime, 14016, 48, std::nullopt, 0);
	expectedSkills.Add(21775, "Aqua Surge (Self)", combatTime, 12954, 3, std::nullopt, 0);
	expectedSkills.Add(31318, "Lunar Impact", combatTime, 12090, 4, std::nullopt, 0);
	expectedSkills.Add(31535, "Ancestral Grace", combatTime, 10976, 4, std::nullopt, 0);
	expectedSkills.Add(29863, "Live Vicariously", combatTime, 8432, 31, std::nullopt, 0);
	expectedSkills.Add(12567, "Nature's Renewal Aura", combatTime, 6862, 47, std::nullopt, 0);
	expectedSkills.Add(21776, "Aqua Surge (Area)", combatTime, 6597, 3, std::nullopt, 0);
	expectedSkills.Add(12836, "Water Blast Combo", combatTime, 4737, 3, std::nullopt, 0);
	expectedSkills.Add(13980, "Windborne Notes", combatTime, 2350, 10, std::nullopt, 0);
	expectedSkills.Add(12825, "Water Blast Combo", combatTime, 1579, 1, std::nullopt, 0);
	...
	for (uint32_t i = 0; i < expectedSkills.Entries.size(); i++)
	{
		...
		AggregatedStatsEntry expected{2000, "Zarwae", combatTime, expectedSkills.Entries[i].Healing, expectedSkills.Entries[i].Hits, std::nullopt, 0 };
...
TEST_P(XevtcLogTestFixture, null_names)
{
	...
	EXPECT_EQ(agentStats.Entries[0].GetTie(),
		AggregatedStatsEntry(2000, "2000", combatTime, 121095, 204, std::nullopt, 0).GetTie());
	...
	{
		const AggregatedVector& skillDetails = stats.GetDetails(DataSource::Skills, expectedSkills.Entries[i].Id);
		AggregatedStatsEntry expected{2000, "2000", combatTime, expectedSkills.Entries[i].Healing, expectedSkills.Entries[i].Hits, std::nullopt, 0 };
		ASSERT_EQ(skillDetails.Entries.size(), 1);
		EXPECT_EQ(skillDetails.HighestHealing, expectedSkills.Entries[i].Healing);
		EXPECT_EQ(skillDetails.Entries[0].GetTie(), expected.GetTie());
	}
