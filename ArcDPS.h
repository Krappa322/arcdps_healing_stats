...
enum cbtstatechange
{
	...
	CBTS_BUFFFORMULA, // (float*)&time[8]: type attr1 attr2 param1 param2 param3 trait_src trait_self, is_flanking = !npc, is_shields = !player, is_offcycle = break, overstack = value of type determined by pad61 (none/number/skill) (not in realtime, one per formula)
...
typedef uintptr_t(*WindowCallbackSignature)(HWND pWindowHandle, UINT pMessage, WPARAM pAdditionalW, LPARAM pAdditionalL);
typedef uintptr_t(*CombatCallbackSignature)(cbtevent* pEvent, ag* pSourceAgent, ag* pDestinationAgent, const char* pSkillname, uint64_t pId, uint64_t pRevision);
typedef uintptr_t(*ImguiCallbackSignature)(uint32_t pNotCharSelectionOrLoading);
